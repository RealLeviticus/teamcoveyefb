...
function classifyNotam(text: string): string {
  ...
  if (/\b(ILS|LLZ|LOC|GS|DME|VOR|DVOR|NDB|TACAN|VORTAC|RNAV|RNP|GNSS|PAPI|VASIS?)\b/.test(t))
    return "Nav Aids";
  if (/\b(ATIS|TWR|APP|ACC|AFIS|FIS|RADAR|COM|FREQ|CPDLC|DATALINK|RCO)\b/.test(t))
    return "Comms/ATC";
  if (/\b(CTR|TMA|AIRSPACE|PROHIBITED|RESTRICTED|DANGER|TRA|TEMP(?:ORARY)? RESERVED)\b/.test(t))
    return "Airspace";
  if (
    /\b(OBST|CRANE|WIP|WORK IN PROGRESS|BIRD|SNOW|ICE|BRAKING ACTION|ARFF|RFFS?|AD\s?CLSD|AERODROME)\b/.test(
      t,
    )
  )
    return "Aerodrome";
  ...
  try {
    ...
    const items: NotamItem[] = nodes.map((n) => {
      const station =
        ...
        "";
      const id =
        n.getAttribute("id") || n.getElementsByTagName("id")?.[0]?.textContent || undefined;
      const starts =
        n.getAttribute("from") || n.getElementsByTagName("from")?.[0]?.textContent || undefined;
      const ends =
        n.getAttribute("to") || n.getElementsByTagName("to")?.[0]?.textContent || undefined;
      const text =
      ...
      for (const tag of candidates) {
        ...
        for (const el of els) {
          ...
          for (const b of blocks) {
            ...
            items.push({
              station: (m?.[0] || "OTHER").toUpperCase(),
              text: b,
              type: classifyNotam(b),
            });
          }
        }
      }
    ...
    return items
      .map((x) => ({
        ...
        id: x.id || undefined,
        text: x.text.trim(),
        type: x.type || classifyNotam(x.text),
      }))
...
function inferOtherPlaceKey(n: NotamItem, wl: WhitelistItem[]): string {
  ...
  const tokens = text
    .toUpperCase()
    .split(/[^A-Z]+/)
    .filter((t) => t.length >= 4 && !UPPER_STOPWORDS.has(t));
...
/* ---------------------- Component ---------------------- */
type Grouped = {
  stations: Map<string, Map<string, NotamItem[]>>;
  otherPlaces: Map<string, Map<string, NotamItem[]>>;
};
...
export function MainArea() {
  ...
  const [acarsType, setAcarsType] = useState<
    "telex" | "cpdlc" | "ping" | "posreq" | "position"
  >("telex");
  const [acarsInbox, setAcarsInbox] = useState<
    Array<{ time?: string; from?: string; to?: string; text: string }>
  >([]);
  ...
  async function acarsLoadInbox() {
    try {
      ...
      if (newCount > 0) {
        if (view === "acars") setAcarsUnread(0);
        else setAcarsUnread((u) => u + newCount);
        try {
          const Ctx =
  ...
  useEffect(() => {
    ...
    const tick = () => {
      if (canPoll) void acarsLoadInbox();
    };
    if (canPoll) {
      try {
        if (acarsInboxKey) {
  ...
  async function resolveLatestPdf(force = false, opts: { silent?: boolean } = {}) {
    ...
    if (!username) {
      if (!silent) setPdfError("Please set your SimBrief username in Settings.");
      return;
    }
    ...
    try {
      const res = await fetch(`/api/simbrief/ofp-latest-pdf${qs}`, {
        cache: "no-store",
      });
      const j = await parseJsonSafe(res);
      if (!res.ok || !j?.ok || !j?.url)
        throw new Error(j?.error || "Invalid server response.");
      const val = await fetch(
        `/api/simbrief/validate-pdf?url=${encodeURIComponent(j.url)}`,
      );
      const vj = await parseJsonSafe(val);
      if (!val.ok || !vj?.ok)
        throw new Error(vj?.error || "PDF could not be validated.");
      setPdfUrl(vj.url);
      localStorage.setItem(LS_PDF, vj.url);
    } catch (e: any) {
      if (!silent) {
        const cached = localStorage.getItem(LS_PDF);
        if (cached) {
          setPdfUrl(cached);
          setPdfError(
            `${e?.message || "Failed"} — showing last known PDF.`,
          );
        } else {
          setPdfError(e?.message || "Failed to load OFP PDF.");
        }
      }
    } finally {
      if (!silent) setLoadingPDF(false);
    }
  }
  ...
  async function resolveFlightSummary(silent = true) {
    ...
    try {
      const res = await fetch(`/api/simbrief/summary${qs}`, {
        cache: "no-store",
      });
      const j = await parseJsonSafe(res);
      if (!res.ok || !j?.ok) throw new Error(j?.error || `HTTP ${res.status}`);
      ...
      if (!silent) setLoadingFlight(false);
    }
  }
  ...
  async function pollVatsimOnce() {
    ...
    try {
      ...
      const gs = Number(
        pilot.groundspeed ?? pilot.ground_speed ?? pilot.gs ?? 0,
      ) || 0;
      const altFt =
        Number(pilot.altitude ?? pilot.altitude_ft ?? pilot.alt ?? 0) || 0;
      const hdg =
        Number(pilot.heading ?? pilot.heading_deg ?? pilot.hdg ?? 0) || 0;

      const squawk = (pilot.transponder ?? pilot.squawk ?? null)
        ? String(pilot.transponder ?? pilot.squawk)
        : null;
      const com = pilot.frequency ?? pilot.com ?? null;

      const depIcao =
        (fp.departure ?? fp.dep ?? pilot.departure)?.toString()?.toUpperCase() ||
        null;
      const arrIcao =
        (fp.arrival ?? fp.arr ?? pilot.arrival)?.toString()?.toUpperCase() ||
        null;
      const altIcao =
        (fp.alternate ?? fp.altn ?? fp.alternate_icao ?? "")
          .toString()
          .toUpperCase() || null;

      const vCallsign =
        (fp.callsign ?? pilot.callsign ?? "").toString().trim() || null;
      const vRoute =
        (fp.route ?? pilot.route ?? "").toString().trim() || null;
      ...
      const acType =
        (
          ...
          .toString()
          .trim() || null;
      const tasKts = parseTasKts(
        fp.tas ?? fp.cruise_tas ?? fp.true_air_speed ?? null,
      );
      const cruiseAltFt = parseCruiseAltFt(
        fp.altitude ?? fp.cruise_altitude ?? null,
      );

      const pilotName =
        (pilot.name ?? pilot.realname ?? "").toString() || null;
      const pilotCid =
        (pilot.cid ?? pilot.vatsim_cid ?? "").toString() || null;
      const airline =
        (pilot.airline ?? callsignAirlinePrefix(vCallsign) ?? "").toString() ||
        null;

      let remainingNm: number | null = null,
        flownNm: number | null = null,
      ...
      if (
        ...
        if (gs > 20 && remainingNm != null) {
          ...
          const m = mins % 60;
          etaText = `${String(h).padStart(2, "0")}:${String(m).padStart(
            2,
            "0",
          )}`;
        }
      ...
      gsHistoryRef.current.push(gs);
      if (gsHistoryRef.current.length > 10) gsHistoryRef.current.shift();
      altHistoryRef.current.push(altFt);
      ...
      setVatsim({
        ...
        route: vRoute,
        origin: depIcao
          ? { icao: depIcao, name: "", lat: null, lon: null }
          : undefined,
        destination: arrIcao
          ? { icao: arrIcao, name: "", lat: null, lon: null }
          : undefined,
  ...
  useEffect(() => {
    ...
    if (view !== "flight" || !keyAvailable) {
      if (vatsimTimer.current) {
        window.clearInterval(vatsimTimer.current);
        vatsimTimer.current = null;
      }
      return;
    }
    void pollVatsimOnce();
    vatsimTimer.current = window.setInterval(
      () => void pollVatsimOnce(),
      15000,
    );
    return () => {
  ...
  useEffect(() => {
    if (username) {
      ...
      void resolveLatestPdf(false, { silent: true });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [username]);
  ...
  useEffect(() => {
    if (view === "ofp" && username && !pdfUrl) {
      void resolveLatestPdf(false);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [view, username]);
  ...
  const fetchNotams = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/simbrief/ofp-notams?username=${encodeURIComponent(username)}`,
        { cache: "no-store" },
      );
      const j = await parseJsonSafe(res);
      let items: NotamItem[] = [];
      ...
      if (!items.length) {
        const xmlRes = await fetch(
          `/api/simbrief/ofp-xml?username=${encodeURIComponent(username)}`,
          { cache: "no-store" },
        );
        if (xmlRes.ok) {
      ...
      items = items.map((n) => ({
        ...n,
        type: n.type || classifyNotam(n.text),
      }));
      setNotams(items);
  ...
  const loadWx = useCallback(async () => {
    try {
      ...
      const res = await fetch(
        `/api/wx/metar-taf?stations=${encodeURIComponent(
          uniq.join(","),
        )}`,
        { cache: "no-store" },
      );
  ...
  /* ---------------------- Grouping ---------------------- */
  const groupedNotams: Grouped = useMemo(() => {
    ...
    for (const n of notams) {
      ...
      if (resolved) {
        if (!result.stations.has(resolved))
          result.stations.set(resolved, new Map());
        ...
        if (!result.otherPlaces.has(placeKey))
          result.otherPlaces.set(placeKey, new Map());
    ...
    const preferred = buildWhitelist(flight).map((w) => w.code);
    ...
    result.otherPlaces = new Map(
      [...result.otherPlaces.entries()].sort(([a], [b]) =>
        a.localeCompare(b),
      ),
    );
  ...
  const zoomOut = () =>
    setZoom((z) => clamp(Math.round((z - 0.1) * 100) / 100, 0.5, 3));
  const zoomIn = () =>
    setZoom((z) => clamp(Math.round((z + 0.1) * 100) / 100, 0.5, 3));
        ...
        >
            ...
            <div className="flex flex-col h-full">
              <div className="flex items-center justify-between px-3 py-2">
                <p className="text-xs opacity-60">
                  OFP PDF (latest{username ? ` — ${username}` : ""})
                </p>
                <div className="flex items-center gap-1">
                  ...
                  <button
                    onClick={() => resolveLatestPdf(true)}
                    disabled={loadingPDF}
                    className="ml-2 text-xs px-2 py-1 rounded-md border bg-white/70 dark:bg-neutral-900/40 hover:bg-white dark:hover:bg-neutral-900 border-neutral-200 dark:border-neutral-700"
                  >
              ...
              {loadingPDF && (
                <div className="px-3 pb-2 text-sm">Resolving PDF link…</div>
              )}
              {pdfError && (
                <div className="px-3 pb-2 space-y-2">
                  <p className="text-red-500 text-sm">Error: {pdfError}</p>
                  <p className="text-xs opacity-70">
                    If the server cannot see your SimBrief session, click{" "}
                    <a
              ...
              <div className="flex-1 min-h-0 overflow-auto">
                {pdfUrl ? (
                  ...
                  !loadingPDF && (
                    <div className="h-full flex items-center justify-center opacity-70 text-sm">
                      {username
                        ? "No PDF link found for the latest OFP."
                        : "Please set your SimBrief username in Settings."}
                    </div>
                  )
                )}
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 h-full">
                <div className="lg:col-span-2 h-full overflow-auto">
                  <div className="mb-3 flex items-center justify-end hidden">
                    <p className="text-xs opacity-60">
                      {username
                        ? `Source: SimBrief — ${username}`
                        : "No SimBrief username set."}
                    </p>
                    <button
                      onClick={() => fetchNotams()}
                      className="text-xs px-2 py-1 rounded-md border bg-white/70 dark:bg-neutral-900/40 hover:bg-white dark:hover:bg-neutral-900 border-neutral-200 dark:border-neutral-700"
                  ...
                  {loadingNotams && (
                    <div className="text-sm">Loading NOTAMs…</div>
                  )}
                  {notamError && (
                    <div className="text-sm text-red-600">{notamError}</div>
                  )}
                    ...
                    <>
                      {(!notams || notams.length === 0) && (
                        <div className="text-sm opacity-70">
                          No NOTAMs found in the OFP.
                        </div>
                      )}

                      {/* Airport buttons (real stations) */}
                      <div className="flex flex-wrap gap-2 mb-3">
                        {Array.from(groupedNotams.stations.keys()).map(
                          (sta) => {
                            const typeMap = groupedNotams.stations.get(sta)!;
                            const count = Array.from(typeMap.values()).reduce(
                              (s, arr) => s + arr.length,
                              0,
                            );
                        ...
                        {groupedNotams.otherPlaces.size > 0 && (
                          ...
                          >
                            OTHER / FIR / Misc (
                            {Array.from(groupedNotams.otherPlaces.values()).reduce(
                              (s, m) =>
                                s +
                      ...
                      {Array.from(groupedNotams.stations.entries()).map(
                        ([station, typeMap]) => {
                          ...
                          return (
                            <section
                              key={station}
                              className="mb-6 rounded-lg border border-neutral-200 dark:border-neutral-800"
                            >
                              <header className="px-3 py-2 border-b border-neutral-200 dark:border-neutral-800 bg-white/60 dark:bg-neutral-900/60">
                                ...
                                <p className="text-xs opacity-60">
                                  {Array.from(typeMap.values()).reduce(
                                    (s, arr) => s + arr.length,
                                    0,
                                  )}{" "}
                                  NOTAMs
                      ...
                      {openStations.has("OTHER") && (
                        <section className="mb-6 rounded-lg border border-neutral-200 dark:border-neutral-800">
                          ...
                          <div className="p-3 flex flex-wrap gap-2">
                            {Array.from(
                              groupedNotams.otherPlaces.entries(),
                            ).map(([place, typeMap]) => {
                              const count = Array.from(typeMap.values()).reduce(
                                (s, arr) => s + arr.length,
                                0,
                ...
                {/* Right-hand column reserved for future use */}
                {false && (
          ...
          {view === "wx" && (
            <div className="h-full overflow-auto">
              ...
              {wxError && (
                <div className="text-sm text-red-600">{wxError}</div>
              )}
              {!loadingWx && !wxError && (
            ...
            <div className="h-full overflow-auto">
              <div className="mb-3 flex items-center justify-between">
                <div className="text-xs opacity-60">
                  From: {acarsFrom || "—"}{" "}
                  {vatsim?.online ? "(VATSIM)" : ""} · Logon:{" "}
                  {acarsLogon || "Not set (Settings)"}
                </div>
                <div className="flex gap-2">
                  <button
                    onClick={() =>
                      setAcarsShowComposer((v) => !v)
                    }
                    className="text-xs px-2 py-1 rounded-md border bg-white/70 dark:bg-neutral-900/40 hover:bg-white dark:hover:bg-neutral-900 border-neutral-200 dark:border-neutral-700"
                  >
                    {acarsShowComposer ? "Close" : "+ New Message"}
                  </button>
                </div>
              </div>
              ...
              {acarsShowComposer && (
                <section className="rounded-lg border border-neutral-200 dark:border-neutral-800 p-3 mb-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-2 mb-2">
                    <div className="md:col-span-2">
                      ...
                      <input
                        value={acarsTo}
                        onChange={(e) =>
                          setAcarsTo(e.target.value.toUpperCase())
                        }
                    ...
                    <div>
                      ...
                      <select
                        value={acarsType}
                        onChange={(e) =>
                          setAcarsType(e.target.value as any)
                        }
                        className="w-full rounded-md border px-3 py-1.5 text-sm bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-700"
                      >
                  ...
                  <div className="flex flex-wrap gap-2 mb-2">
                    ...
                    <button
                      onClick={useTemplateCpdlcLogon}
                      className="text-[11px] px-2 py-1 rounded-md border bg-white/70 dark:bg-neutral-900/40 hover:bg-white dark:hover:bg-neutral-900 border-neutral-200 dark:border-neutral-700"
                    >
                      + CPDLC Logon
                    </button>
                    <button
                      onClick={useTemplatePosition}
                      className="text-[11px] px-2 py-1 rounded-md border bg-white/70 dark:bg-neutral-900/40 hover:bg-white dark:hover:bg-neutral-900 border-neutral-200 dark:border-neutral-700"
                    >
                      + Position Report
                    </button>
                  ...
                  <textarea
                    ...
                    rows={6}
                    className="w-full rounded-md border px-3 py-1.5 text-sm bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-700 mb-2"
                  />
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => void acarsSend()}
                      disabled={acarsSending}
